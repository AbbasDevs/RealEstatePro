...
export class TenantSettingsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.tenantService.getTenant(id).subscribe({
      next: t => {
        this.form.patchValue({ name: t.name, email: t.email, phoneNumber: t.phoneNumber });
  ...
  save(): void {
    ...
    this.saving = true;
    this.tenantService.updateTenant(id, this.form.value as any).subscribe({
      next: () => {
        this.snack.open('Settings saved!', 'Close', { duration: 2000 });
        this.saving = false;
      },
